...
namespace ge {
	namespace clientcore {
		...
		struct BufferElement {
			...
			BufferElement(ShaderDataType type, const String& name, bool normalized = false): name(name), type(type), size(shaderDataTypeSize(type)), offset(0), normalized(normalized) {
			}
		...
		public:
			...
			inline const ge::core::List<BufferElement>& getElements() const { return elements; }
		...
		private:
			...
			ge::core::List<BufferElement> elements;
			uint32 stride;
		};
		...
		public:
			...
			static ge::core::Ref<IVertexBuffer> create(size_t size);
